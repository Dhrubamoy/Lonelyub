...
BOT_IS_UP_TO_DATE = "**The Lonely** is up-to-date sir."
NEW_BOT_UP_DATE_FOUND = (
    "new update found for {branch_name}\n"
    "changelog: \n\n{changelog}\n"
    "updating your Lonely ..."
)
NEW_UP_DATE_FOUND = (
    "New update found for {branch_name}\n" "`updating your Lonely...`"
)
...
async def lonely_info(lonely_info):
    infos = requests.get(legendbot_info).json()
    _version = infos["LONELY-INFO"]["version"]
    _release = infos["LONELY-INFO"]["release-date"]
    _branch = infos["LONELY-INFO"]["branch"]
    _author = infos["LONELY-INFO"]["author"]
    _auturl = infos["LONELY-INFO"]["author-url"]
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n**😎 Lonely is UP-TO-DATE.**"
            f"\n\n**Version :**  {LonelyVersion}"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"🌚 Do `.update build` to update your **Lonely** !!"
        )

    if force_update:
        await event.edit(
            "\n**😎 Loneltbo† is UP-TO-DATE.**"
            f"\n\n**Version :**  {LonelyVersion}"
            f"\n**Owner :**  {legend_mention}"
            f"\n**Git Branch :**  {UPSTREAM_REPO_BRANCH}\n"
        )
...
async def upstream(event):
    ...
    off_repo = "https://github.com/Dhrubamoy/LonelyBot"
    os.chdir("/app")
